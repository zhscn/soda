...
| 嵌入 Chez runtime boot 和编译后的 editor boot | 已实现 |
...
Scheme editor program
  -> whole-program object
  -> soda-editor.boot
  -> generated byte array

launcher + generated boot arrays
  -> soda executable
```

C 入口完成 native foreign symbol 注册、boot 注册、heap 建立和进程参数准备，然后
调用 Scheme 入口。Scheme editor boot 持有 command loop；launcher 不实现编辑器
策略，也不解析 Scheme library。

`soda-chez.boot` 包含 Chez compiler，因此运行中的编辑器可以使用 `eval`、
`compile-program` 和 debugger，不依赖外部 Chez 安装。editor boot 由

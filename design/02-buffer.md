...
最终文件路径通过 `file.read` effect 提交。请求携带可选的发起 View identity 与
可选目标 byte offset；没有 View identity 的请求把资源打开为后台 Buffer，不改变
窗口或活动 View。runtime adapter 先异步 stat 路径，再按结果进入目录、读取或
...
结果创建带 `file_path` 的 Buffer，根据路径选择初始 major mode，恢复请求中存在的
各 View 目标位置，并记录原始换行约定。成功 stat 的文件状态随读取结果进入 Buffer，
作为后续保存的版本前提。

...
struct Runtime::Impl {
    ...
    SourceId spawn_process(std::vector<std::string> arguments, std::string working_directory) {
        ...
        stdio[0].data.stream = reinterpret_cast<uv_stream_t*>(&operation.standard_input.handle);
        stdio[1].flags =
            static_cast<uv_stdio_flags>( // NOLINT(clang-analyzer-optin.core.EnumCastOutOfRange)
    ...
    SourceId spawn_terminal_process(std::vector<std::string> arguments,
        ...
        if (rows == 0 || columns == 0 || rows > std::numeric_limits<unsigned short>::max() ||
    ...
    void write_process(SourceId source, std::vector<std::byte> data) {
        ...
        const uv_buf_t buffer = uv_buf_init(reinterpret_cast<char*>(write->data.data()),
                                            static_cast<unsigned int>(write->data.size()));
        uv_stream_t* stream = process.terminal_mode
                                  ? reinterpret_cast<uv_stream_t*>(&process.terminal.handle)
                                  : reinterpret_cast<uv_stream_t*>(&process.standard_input.handle);
        const int status = uv_write(&write->request, stream, &buffer, 1, on_process_input_written);
        if (status < 0) {
    ...
    void finish_process_if_closed(Process& process) {
        const bool streams_closed = process.terminal_mode ? process.terminal.closed
    ...
    static void on_process_input_written(uv_write_t* request, int status) noexcept {
        guard_callback([&] {
            std::unique_ptr<Process::Write> write(static_cast<Process::Write*>(request->data));
...
SourceId Runtime::spawn_terminal_process(std::vector<std::string> arguments,
                                         std::string working_directory, std::uint32_t rows,
                                         std::uint32_t columns) {
    return impl_->spawn_terminal_process(std::move(arguments), std::move(working_directory), rows,
                                         columns);
}
...
void Runtime::resize_process_terminal(SourceId source, std::uint32_t rows, std::uint32_t columns) {

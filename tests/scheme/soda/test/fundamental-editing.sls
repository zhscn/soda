(library (soda test fundamental-editing)
  ...
  (define (run-fundamental-editing-tests!)
    ...
    (let* ([application (make-soda-application)]
      ...
      ;; A terminal read may hold several legacy C-n bytes.  Each command
      ;; still receives the previous command's ViewState, while the frontend
      ;; presents only the final result of this one input burst.
      (let ([decoder (make-terminal-input-decoder)])
        ...
        (unless (= (selection-range-head
                   ...
                   6)
          (error 'fundamental-editing-tests
                 "input burst did not apply every C-n motion"))
